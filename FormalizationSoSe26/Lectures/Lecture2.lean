...
2. We will use it exclusively for mathematics and theorem proving.
   ...
   We have the type `Prop` for propositions. These are the *truth values*.
...
#check (ULift.{3} : (Type 1) → (Type 3))

-- Functions valued in `Type` are *indexed sets*.
#check ℝ → Type

-- Functions valued in `Prop` are *formulas*.
#check ℕ → Prop
...
/-
One important aspect of mathematics are ``dependent types``.
These are functions where the codomain depends on the input.

Here is an example:
-/
def identityN := fun n : ℕ => (@rfl _ n)

#check (identityN 5)
/-
Notice `identityN 5` is not a function, but a `dependent function`.
The codomain of `identityN n` is the type `n = n`, which depends on `n`.

A common syntax for such dependent functions is the following:
-/

def identityN' (n : ℕ) := @rfl _ n
/-
So here `n : ℕ` is the input, but we also think of `@rfl _ n` as *depending* on `n : ℕ`.
-/
...
#print Even
/-
By definition a term in `Even n` is a pair `⟨k, hk⟩` where `k : ℕ` and `hk : n = 2 * k`.
-/

/-
Here are three different ways to write the same statement.
Notice various levels of the dependent function and types:
-/
example : ∀ m n : Nat, Even n → Even (m * n) := sorry
example (m n : Nat) : Even n → Even (m * n) := sorry
example (m n : Nat) (_ : Even n) : Even (m * n) := sorry
...
Naively, we should have
- `l = m * k`,
- `hl` is the combination of `mul_add m k k` and `m * hk`.
...
example : ∀ m n : Nat, Even n → Even (m * n) := by
  ...
  -- Substitute for `n`,
  rw [hk]
  -- and now it is one last equality.
  rw [mul_add]

/-
Here is a combination method combining functions and tactics.
-/
example : ∀ m n : Nat, Even n → Even (m * n) :=
  fun m n ⟨k, hk⟩ ↦
...
/-
Here is a more compact way to use tactics.
-/
example : ∀ m n : Nat, Even n → Even (m * n) := by
...
-- We use `\ l` to obtain the symbol `←` to rewrite in the other direction.
...
example (P Q : Prop) (h : P) (k : P → Q) : Q := k h
